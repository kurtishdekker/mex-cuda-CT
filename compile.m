...
% forward projection operations
nvccCommandLine = ['nvcc -c CUDAmex_general3D_BP.cu -o CUDAmex_general3D_FP.o -O3 ' gpu_compute ' --ptxas-options=-v -I"' matlab_path '" -l"' vs_path '\lib" -ccbin "' vs_path '\bin"'];
status = system(nvccCommandLine);
if status < 0
	error 'Error invoking nvcc'
end
...
mex('-largeArrayDims','CUDAmex_FP.o', ['-L' cuda_path],'-lcudart',['-outdir ' user_matlab_path]);	
mex('-largeArrayDims','CUDAmex_BP.o', ['-L' cuda_path],'-lcudart',['-outdir ' user_matlab_path]);	
mex('-largeArrayDims','CUDAmex_general3D_FP.o', ['-L' cuda_path],'-lcudart',['-outdir ' user_matlab_path]);	
mex('-largeArrayDims','CUDAmex_general3D_BP.o', ['-L' cuda_path],'-lcudart',['-outdir ' user_matlab_path]);	
...
% compile the object files into .mex files
mex('/NODEFAULTLIB:MSVCRT.lib','-largeArrayDims', 'CUDAmex_oscIter.o', ['-L' cuda_path], '-lcudart',['-outdir ' user_matlab_path]);
mex('/NODEFAULTLIB:MSVCRT.lib','-largeArrayDims', 'CUDAmex_oscIter.o', ['-L' cuda_path], '-lcudart',['-outdir ' user_matlab_path]);
